...
namespace ICSharpCode.CodeCompletion {
	public class CSharpCompletion {
		...
		public CSharpCompletion(IReadOnlyList<Assembly> assemblies = null) {
			...
			var unresolvedAssemblies = new List<IUnresolvedAssembly>();
			...
			Parallel.For(
				...
				delegate (int i) {
					...
					if (string.IsNullOrWhiteSpace(path)) {
						projectContent = projectContent.AddAssemblyReferences(new DefaultAssemblyReference(assemblies[i].FullName));
					}
					else {
						loader.DocumentationProvider = GetXmlDocumentation(path);
						unresolvedAssemblies.Add(loader.LoadAssemblyFile(path));
					}
				});
		...
		public void AddAssembly(byte[] data) {
			using (MemoryStream stream = new MemoryStream(data)) {
				...
				var loader = new CecilLoader();
				var unresolvedAssembly = AssemblyDefinition.ReadAssembly(stream);
