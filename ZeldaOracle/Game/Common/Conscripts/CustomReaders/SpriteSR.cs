	...
	public partial class SpriteSR : ConscriptRunner {
		...
		public SpriteSR() {
			...
			delegate (CommandParam parameters) {
				...
				if (Resources.Contains<Image>(imagePath))
			...
			delegate (CommandParam parameters) {
				AddSprite(parameters.GetString(0), GetSpriteFromParams(parameters, 1));
			});
			//=====================================================================================
			AddBasicCommands();
		...
		protected override void OnBeginReading() {
			...
			paletteArgs = new SpritePaletteArgs();
			paletteArgs.ColorMapping = new Dictionary<Color,
				Dictionary<int, ColorGroupSubtypePair>>();
			paletteArgs.IndexedPossibleColorGroups = new int[0];
			paletteArgs.PossibleColorGroups = new string[0];
			paletteArgs.IgnoreColors = new HashSet<Color>();

			animationBuilder = new AnimationBuilder();
			editingSpriteSet = null;
			editingStyleGroup = "";
			editingColorGroups = new List<string>();
			editingColorStyleGroups = new List<ColorStyleGroup>();
		}
		...
		/// <summary>Adds the sprite to the sprite and animation lists.</summary>
		private void AddSprite(string name, ISprite sprite) {
			AddResource<ISprite>(name, sprite);
			if (sprite is Animation)
				AddResource<Animation>(name, (Animation) sprite);
		}

		/// <summary>Sets the sprite for the sprite and animation lists.</summary>
		private void SetSprite(string name, ISprite sprite) {
			SetResource<ISprite>(name, sprite);
			if (sprite is Animation)
				SetResource<Animation>(name, (Animation) sprite);
		}
		...
		private ISprite GetSprite(ISpriteSource source, Point2I index) {
			...
			if (source is SpriteSheet && paletteArgs.Dictionary != null) {
				...
				return Resources.SpriteDatabase.AddSprite(paletteArgs);
			}
			else {
		...
		private BasicSprite BuildBasicSprite(ISpriteSource source, Point2I index, Point2I drawOffset, Flip flip = Flip.None, Rotation rotation = Rotation.None) {
			...
			if (paletteArgs.Dictionary != null) {
				...
				BasicSprite sprite = Resources.SpriteDatabase.AddSprite(paletteArgs);
