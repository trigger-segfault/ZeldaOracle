 ...
 * </summary> */
public class PropertyEmitterList : Property {
	...
	protected static List<ParticleEmitter> emitters = null;
	...
	/** <summary> Constructs the default property. </summary> */
	public PropertyEmitterList(string name, ParticleEmitter value, ParticleEmitter defaultValue, PropertyAction action = null)
	...
	/** <summary> Constructs the default property. </summary> */
	public PropertyEmitterList(string name, int maxRows, ParticleEmitter value, ParticleEmitter defaultValue, PropertyAction action = null)
		: base(name, value, defaultValue, action) {
	...
	public int Selection {
		...
		set {
			selection	= value;
			if (selection >= 0 && selection < emitters.Count) {
				text		= emitters[value].Name;
				this.value	= emitters[value];
			}
		}
	}
	/** <summary> Gets the name of the current selection. </summary> */
	public string SelectionName {
		get { return ((selection >= 0 && selection < emitters.Count) ? emitters[selection].Name : ""); }
	}
	/** <summary> Gets the value of the current selection. </summary> */
	public ParticleEmitter SelectionValue {
		get { return ((selection >= 0 && selection < emitters.Count) ? emitters[selection] : null); }
	}
	...
	public static void UpdateList() {
		...
		emitters.AddRange(Resources.ParticleEmitters);
	}
	...
	public override void Update(double time, Point2I position) {
		...
		if (selection >= emitters.Count || selection == -1 || emitters[selection] != value) {
			selection = -1;
			...
			if (selection == -1) {
				selection = 0;
				text = emitters[0].Name;
			}
		}
		if (emitters[selection].Name != text) {
			text = emitters[selection].Name;
		}
		...
		if (valueRect.Contains(mousePos) && Mouse.IsButtonPressed(MouseButtons.Left)) {
			...
			g.emitterChanged = true;
			g.propEmitterSelection.Selection = selection;
		}

		int listWidth = 200;

		for (int i = 0; i < emitters.Count; i++) {
			listWidth = (int)GMath.Max(listWidth, grid.Font.MeasureString(emitters[i].Name).X + TextPadding * 3);
		}
		...
		if (dropDown) {
			...
			for (int i = 0; i < emitters.Count && i < MaxListRows; i++) {
				if (itemRect.Contains(mousePos) && Mouse.IsButtonPressed(MouseButtons.Left)) {
					selection = i + listPosition;
					text = emitters[i + listPosition].Name;
					value = emitters[i + listPosition];
					dropDown = false;
					clickedInside = true;
					changed = true;
					if (action != null)
		...
		if (dropDown) {
			...
			for (int i = 0; i < emitters.Count && i < MaxListRows; i++) {
				...
				if (selection == i + listPosition) {
					...
					g.DrawString(grid.Font, emitters[i + listPosition].Name, new Point2I(itemRect.X + TextPadding, (int)itemRect.Center.Y), Align.Left | Align.Int, colorTextHighlight);
				}
				else if (itemRect.Contains(mousePos)) {
					g.FillRectangle(itemRect.Inflated(-2, -2), colorBackgroundHighlight);
					g.DrawString(grid.Font, emitters[i + listPosition].Name, new Point2I(itemRect.X + TextPadding, (int)itemRect.Center.Y), Align.Left | Align.Int, colorText);
				}
				else {
					g.DrawString(grid.Font, emitters[i + listPosition].Name, new Point2I(itemRect.X + TextPadding, (int)itemRect.Center.Y), Align.Left | Align.Int, colorText);
				}
