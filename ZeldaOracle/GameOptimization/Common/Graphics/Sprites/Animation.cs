...
namespace ZeldaOracle.Common.Graphics.Sprites {
	...
	public enum LoopMode {
		...
		Reset,
		/// <summary>Keep looping back and playing from the beginning endlessly.
		/// Same as Loop.</summary>
		Repeat,
		/// <summary>Keep looping back and playing from the beginning endlessly.
		/// Same as Repeat.</summary>
		Loop,
	}


	/// <summary>A sprite with different combinations of smaller sprites for each
	/// frame.</summary>
	public class Animation : ISprite {
		...
		/// <summary>This creates a linked list of animations for its variations
		/// (like for different directions).</summary>
		...
		public Animation(ISprite sprite) {
			...
			AddFrame(new AnimationFrame(0, 1, sprite));
		}
		...
		public SpritePart GetParts(SpriteSettings settings) {
			...
			for (int i = 0; i < frames.Count; ++i) {
				...
				if (time >= frame.StartTime &&
					(time < frame.EndTime || (time >= duration &&
					frame.StartTime + frame.Duration == duration)))
				{
		...
		public Rectangle2I GetBounds(SpriteSettings settings) {
			...
			foreach (AnimationFrame frame in frames) {
				...
				if (bounds.IsEmpty)
					...
					bounds = Rectangle2I.Union(bounds,
						frame.OffsetSprite.GetBounds(settings));
			}
			return bounds;
		}
		...
		/// <summary>Gets the last frame in the list or returns null if the list is
		/// empty.</summary>
		...
		public void AddFrame(AnimationFrame frame) {
			if (frame.OffsetSprite != null &&
				frame.OffsetSprite.ContainsSubSprite(this))
				throw new SpriteContainmentException(this, "AnimationFrame");
			int index = 0;
			while (index < frames.Count && frame.Depth >= frames[index].Depth) {
				if (frame.StartTime < frames[index].StartTime &&
					frame.Depth == frames[index].Depth)
					break;
				index++;
			}
		...
		{
			AddFrame(new AnimationFrame(startTime, duration, sprite, clipping,
				flip, rotation, depth));
		}
		...
		public void AddFrame(int startTime, int duration, ISpriteSource source,
			Point2I index, Rectangle2I? clipping = null, Flip flip = Flip.None,
			Rotation rotation = Rotation.None, int depth = 0)
		{
			AddFrame(new AnimationFrame(startTime, duration, source, index, clipping,
				flip, rotation, depth));
		}

		/// <summary>Adds a frame with the specified settings.</summary>
		public void AddFrame(int startTime, int duration, ISpriteSource source,
			Point2I index, string definition, Rectangle2I? clipping = null,
			Flip flip = Flip.None, Rotation rotation = Rotation.None, int depth = 0)
		{
			AddFrame(new AnimationFrame(startTime, duration, source, index, definition,
				clipping, flip, rotation, depth));
		}

		/// <summary>Adds a frame with the specified settings.</summary>
		public void AddFrame(int startTime, int duration, ISpriteSource source,
			Point2I index, Point2I drawOffset, Rectangle2I? clipping = null,
			Flip flip = Flip.None, Rotation rotation = Rotation.None, int depth = 0)
		{
		...
		/// <summary>Adds a frame with the specified settings.</summary>
		public void AddFrame(int startTime, int duration, ISpriteSource source,
			Point2I index, string definition, Point2I drawOffset,
			Rectangle2I? clipping = null, Flip flip = Flip.None,
			Rotation rotation = Rotation.None, int depth = 0)
		{
			AddFrame(new AnimationFrame(startTime, duration, source, index, definition,
				drawOffset, clipping, flip, rotation, depth));
		}
		...
		public CompositeSprite GetFrameAsCompositeSprite(int time) {
			...
			for (int i = 0; i < frames.Count; i++) {
				if (frames[i].StartTime <= time &&
					frames[i].StartTime + frames[i].Duration - 1 >= time)
				{
