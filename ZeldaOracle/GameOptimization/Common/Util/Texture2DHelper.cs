	...
	public static class Texture2DHelper {
		...
		public static void PremultiplyAlpha(Texture2D texture) {
			...
			using (RenderTarget2D renderTarget = new RenderTarget2D(
				ContentContainer.GraphicsDevice, texture.Width, texture.Height))
			{
				ContentContainer.GraphicsDevice.SetRenderTarget(renderTarget);
				ContentContainer.GraphicsDevice.Clear(XnaColor.Black);

				// Multiply each color by the source alpha, and write
				// in just the color values into the final texture
				ContentContainer.SpriteBatch.Begin(SpriteSortMode.Immediate,
					BlendColorBlendState);
				ContentContainer.SpriteBatch.Draw(texture, texture.Bounds,
					XnaColor.White);
				ContentContainer.SpriteBatch.End();
				...
				ContentContainer.GraphicsDevice.SetRenderTarget(null);
				...
				ContentContainer.GraphicsDevice.Textures[0] = null;
				texture.SetData<XnaColor>(data);
			}
		}
	}
