...
namespace ZeldaOracle.Game.API {
	...
	public class Variables {
		...
		/// and type.</summary>
		public bool Contains(string name, VariableType type) {
		...
		public Variable SetEnum<E>(string name, E value,
			VariableType defaultType = VariableType.String) where E : struct
		{
			Variable v = GetVariable(name);
			VariableType type = defaultType;
			if (v != null)
				type = v.Type;
		...
		public Variable AddBuiltInEnum<E>(string name, E value,
			VariableType type = VariableType.String) where E : struct
		{
			if (type == VariableType.Integer)
